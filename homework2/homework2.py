...
Original file is located at
    https://colab.research.google.com/drive/1-qYHntCbQODG01G9imPCq2fCru7_2nlB
...
def extract_label(path):
    # Your code here
    filename=path.replace("\\","/").split('/')[-1].split('_')[0]
    return INSTRUMENT_MAP[filename]
...
w0,_=librosa.load(audio_paths[0], sr=SAMPLE_RATE)
...
def extract_mfcc(w):
    ...
    # concatenate
    mfccs = librosa.feature.mfcc(y=w, sr=SAMPLE_RATE, n_mfcc=N_MFCC)
    mfcc_mean = np.mean(mfccs, axis=1)
    mfcc_std = np.std(mfccs, axis=1)
    features = np.concatenate((mfcc_mean, mfcc_std))
    return torch.FloatTensor(features)

# print(extract_mfcc(w0))
...
def extract_spec(w):
    ...
    stft_complex = librosa.stft(w)
    spec = np.abs(stft_complex) ** 2
    # w_tensor = torch.from_numpy(w)
    # stft = torch.stft(w_tensor, n_fft=1024, hop_length=512, window=torch.hann_window(1024), return_complex=True)
    # spec = torch.abs(stft) ** 2
    return spec

# print(extract_spec(w0))
...
def extract_label_7(path):
    # Your code here
    filename = '_'.join(path.replace("\\","/").split('/')[-1].split('_')[:2])
    return INSTRUMENT_MAP_7[filename]
# print(extract_label_7(audio_paths[0]))
